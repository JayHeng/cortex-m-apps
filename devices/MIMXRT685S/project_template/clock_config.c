...
const clock_audio_pll_config_t g_configAudioPll = {
    .audio_pll_src = kCLOCK_AudioPllXtalIn, .numerator = 14, .denominator = 75, .mult = 22U};
...
AT_QUICKACCESS_SECTION_CODE(static void BOARD_SetQspiClock(QuadSPI_Type *qspi, uint32_t qspiClockSrc, uint32_t divider))
{
    int32_t i;
    ...
    if (qspi)
    {
        ...
        /* Update qspi clock. */
        CLKCTL0->OSPIFFCLKSEL = qspiClockSrc;
...
void BOARD_BootClockRUN(void)
{
     ...
     * PLL and main clock.
     */
    BOARD_SetQspiClock(qspi, CLKCTL0_OSPIFFCLKSEL_SEL(3), 1);
    ...
    CLOCK_InitSysPfd(kCLOCK_Pfd1, 16); /* Enable dsp PLL clock 594MHz. */
    CLOCK_InitSysPfd(kCLOCK_Pfd2, 24); /* Enable aux0 PLL clock 396MHz for SDIO/QSPI */

    CLOCK_InitAudioPll(&g_configAudioPll); /* Configure audio PLL to 528Mhz. */

    CLOCK_InitAudioPfd(kCLOCK_Pfd0, 26);        /* Enable audio PLL PFD0 271MHz */
    CLOCK_SetClkDiv(kCLOCK_DivAudioPllClk, 15); /* Configure audio_pll_clk to 12.34Mhz */
    ...
    /* Let QSPI run on aux0 pll clock (396/1 = 396MHz) for MX25UM51345G flash. This
     * results in octal DDR read at 100MHz I/O speed.
     ...
     */
    BOARD_SetQspiClock(qspi, CLKCTL0_OSPIFFCLKSEL_SEL(2), 1);
