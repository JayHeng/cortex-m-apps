                                              ...
                                              uint32_t keySize)
{
    ...
    /* Currenty we do not support EXTENDED lenght in FIFO LOAD/STORE command.
     * Data lenght limit is 2^16 bytes = 65 536 bytes.
     * Note: While computing HASH, try calling CAAM_HASH_Update() several times instead
     */
    if (dataSize > 0xFFFFUL)
    {
